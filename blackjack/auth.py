...
def login():
    if request.method == 'POST':
        ...
        if error is None:
            ...
            dbcur.execute(
                'CALL getlastsession(%s)',(session['user_id'],))
            max = dbcur.fetchone()
            if max[0] is None:
...
def logout():
    ...
    dbcur = db.cursor()
    dbcur.execute(
        'CALL insertstats(%s,%s,%s,%s,%s,%s,%s)',(session['session_id'],session['user_id'],dateToday, session['wins'], session['losses'], session['ties'],session['totalRounds'],)
    )
    db.commit()
    db.close()
    session.clear()
